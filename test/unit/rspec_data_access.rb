    ...
    describe '#updateStock' do
        ...
        context "book is new" do
          it "should add it to database but leave remote cache unchanged" do
              ...
              expect(@sqlite_database).to receive(:updateStock).with('5555').and_return(@book1)
          
          # expect(@sqlite_database).to receive(:updateStock).with(0);
            expect(@dalli_client).to receive(:get).with("v_#{@book1.isbn}" ).
                   and_return(nil)
        ...
        context "book is new" do
           it "should add it to database and added book into two caches" do
              expect(@sqlite_database).to receive(:updateStock).with('5555').and_return(@book1)
              expect(@dalli_client).to receive(:set).with("v_#{@book1.isbn}",3)
              expect(@dalli_client).to receive(:set).with("#{@book1.isbn}_3",@book1.to_cache )                 
      ...
      context "book is not new and related book is in the remote cache" do
        it "should update in the remote cache and change quantity in database" do  
        expect(@sqlite_database).to receive(:updateStock).with(@book_from_database1)
          expect(@dalli_client).to receive(:get).with("v_#{@book1.isbn}" ).
                   and_return(2)
            expect(@dalli_client).to receive(:set).with("v_#{@book1.isbn}",3)
            expect(@dalli_client).to receive(:set).with("#{@book1.isbn}_3",@book1.to_cache )                 
